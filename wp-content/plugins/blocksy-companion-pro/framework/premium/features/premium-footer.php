...
class PremiumFooter {
	public function __construct() {
		...
		add_filter('blocksy:footer:items-config', function ($config, $item_id) {
			if ($item_id === 'menu') {
				$config['name'] = __('Footer Menu 1', 'blc');
			}
		...
		add_filter('blocksy:register_nav_menus:input', function ($items) {
			...
			$items['footer'] = __('Footer Menu 1', 'blc');
			$items['footer_2'] = __('Footer Menu 2', 'blc');
		...
		add_filter('blocksy:footer:selective_refresh', function ($selective_refresh) {
			...
			$selective_refresh[] = [
				...
				'fallback_refresh' => false,
				'container_inclusive' => true,
				'selector' => '.ct-footer [data-id="contacts"]',
				'settings' => ['footer_placements'],
				'render_callback' => function () {
					$footer = new \Blocksy_Footer_Builder_Render();
	...
	public function get_conditions() {
		$option = get_theme_mod('blocksy_premium_footer_conditions', [
			[
				'id' => 'type-1',
				'conditions' => [
					[
