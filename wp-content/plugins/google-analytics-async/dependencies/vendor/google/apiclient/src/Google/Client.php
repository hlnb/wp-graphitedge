...
class Google_Client
{
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \Beehive\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\Beehive\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \Beehive\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\Beehive\GuzzleHttp\ClientInterface $http = null)
    {
        $credentials = null;
        $token = null;
        $scopes = null;
    ...
    {
        $tokenRevoker = new \Beehive\Google_AccessToken_Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \Beehive\Google_AccessToken_Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
     ...
     */
    public function execute(\Beehive\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \Beehive\Google_Http_REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \Beehive\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
     ...
     */
    public function setCache(\Beehive\Psr\Cache\CacheItemPoolInterface $cache)
    {
        $this->cache = $cache;
    }
    /**
     ...
     */
    public function setLogger(\Beehive\Psr\Log\LoggerInterface $logger)
    {
        $this->logger = $logger;
    }
    /**
    ...
    {
        $logger = new \Beehive\Monolog\Logger('google-api-php-client');
        if ($this->isAppEngine()) {
            $handler = new \Beehive\Monolog\Handler\SyslogHandler('app', \LOG_USER, \Beehive\Monolog\Logger::NOTICE);
        } else {
            $handler = new \Beehive\Monolog\Handler\StreamHandler('php://stderr', \Beehive\Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \Beehive\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
     * Set the Http Client object
     * @param GuzzleHttp\ClientInterface $http
     */
    public function setHttpClient(\Beehive\GuzzleHttp\ClientInterface $http)
    {
        $this->http = $http;
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        if (\defined('\\Beehive\GuzzleHttp\ClientInterface::MAJOR_VERSION')) {
            $guzzleVersion = \Beehive\GuzzleHttp\ClientInterface::MAJOR_VERSION;
        } elseif (\defined('\\Beehive\GuzzleHttp\ClientInterface::VERSION')) {
            $guzzleVersion = (int) \substr(\Beehive\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        ...
        if (5 === $guzzleVersion) {
            ...
            if ($this->isAppEngine()) {
                ...
                $options['handler'] = new \Beehive\GuzzleHttp\Ring\Client\StreamHandler();
        ...
        return new \Beehive\GuzzleHttp\Client($options);
    }
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \Beehive\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            $credentials = \Beehive\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, null, null, $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \Beehive\Google\Auth\Credentials\ServiceAccountCredentials) {
    ...
    protected function getAuthHandler()
    {
        ...
        return \Beehive\Google_AuthHandler_AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        ...
        return new \Beehive\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
/**
 * The Google API Client
 * https://github.com/google/google-api-php-client
 */
\class_alias('Beehive\\Google_Client', 'Google_Client', \false);