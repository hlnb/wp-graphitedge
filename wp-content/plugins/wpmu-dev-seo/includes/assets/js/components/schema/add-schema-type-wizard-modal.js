...
export default class AddSchemaTypeWizardModal extends React.Component {
	...
	getTypeLabelFull(type) {
		if (!schemaTypes.hasOwnProperty(type)) {
			return type;
		}

		return schemaTypes[type].labelFull || schemaTypes[type].label;
	}
	...
	breadcrumbs() {
		...
		if (types.length) {
			return <div id="wds-add-schema-type-breadcrumbs">
				{types.map((type) => <span>{this.getTypeLabelFull(type)} <span className="sui-icon-chevron-right"
